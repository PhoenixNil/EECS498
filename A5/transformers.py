...
def prepocess_input_sequence(
    ...
    for token in input_str.split():
        if token in spc_tokens:
            ...
            out.extend([token_dict[t] for t in token])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        weighed = torch.zeros(K)
        for j in range(K):
            weighed[j] = torch.dot(query[i], key[j]) / \
                torch.sqrt(torch.tensor(M))  # scaled dot product
        weighted_sum = torch.matmul(
            F.softmax(weighed, dim=0), value)  # weighted sum of values
        out[i] = weighted_sum
...
def scaled_dot_product_two_loop_batch(
    ...
    for i in range(N):
        for j in range(K):
            # scaled dot product,第i个batch的第j个query和所有的key做点积,再除以sqrt(M)，即第i个batch的第j个query对所有的key的权重
            weighed = torch.matmul(
                query[i][j], key[i].T)/torch.sqrt(torch.tensor(M))
            weighted_sum = torch.matmul(
                F.softmax(weighed, dim=0), value[i])  # weighted sum of values

            out[i][j] = weighted_sum  # 第i个batch的第j个query的输出
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    weights_softmax = torch.bmm(query, key.transpose(
        1, 2)) / torch.sqrt(torch.tensor(M))  # scaled dot product
    y = torch.bmm(F.softmax(weights_softmax, dim=2),
                  value)  # weighted sum of values
    if mask is not None:
        ##########################################################################
        ...
        # Replace "pass" statement with your code
        mask = mask.float()
        mask = mask.masked_fill(mask == True, -1e9)
    # Replace "pass" statement with your code
        weights_softmax = weights_softmax + mask
        y=torch.bmm(F.softmax(weights_softmax, dim=2),value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        # initialize the weights sampled from a uniform distribution bounded by [-c, c]
        c = torch.sqrt(torch.tensor(6 / (dim_in + dim_q)))
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.heads = nn.ModuleList([SelfAttention(dim_in, dim_out, dim_out) for _ in range(
            num_heads)])  # 初始化num_heads个SelfAttention block
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # 用每个head对query, key, value, mask进行处理
        heads = [head(query, key, value, mask) for head in self.heads]
        y = torch.cat(heads, dim=2)  # 将每个head的输出拼接起来
        y = self.linear(y)  # 将拼接后的输出映射回dim_in
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        # Compute the standard deviation from variance and add epsilon for numerical stability
        std = torch.sqrt(var + self.epsilon)

        # Normalize the input
        x_normalized = (x - mean) / std

        # Scale and shift this normalized input using gamma and beta
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # 每个头处理的维度是整体embedding维度的一部分，即emb_dim / num_heads,然后concat起来,所以输出维度是emb_dim
        self.multi_head_attention = MultiHeadAttention(
            num_heads, emb_dim,  int(emb_dim/num_heads))
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # Use the input thrice as specified in self-attention mechanism
        ...
        # residual connection and layer normalization
        out3 = self.layer_norm2(out3 + out2)
...
def get_subsequent_mask(seq):
    ...
    N, K = seq.shape
    # 创建一个全为 1 
    ones = torch.ones((K, K),device = seq.device)

    # 通过将对角线偏移1并应用triu函数生成子序列掩码
    # 这将把所有未来的位置（对角线以上）标记为True（1）
    mask = ones.triu(diagonal=1).bool()

    # 将掩码复制N次，以便它的形状与输入序列相同
    mask = mask.repeat(N, 1, 1) 

    #这是使用位操作来生成掩码
    # N, K = seq.shape
    # mask = torch.zeros((N, K, K), dtype=torch.bool, device=seq.device)
    # for i in range(K):
    #     # 使用位移操作生成每一行的掩码
    #     binary_num = 2 ** (K - i - 1) - 1
    #     binary_num = list(map(int, list(format(binary_num, '0{}b'.format(K)))))
    #     # 将这个掩码复制N次，以便它的形状与输入序列相同
    #     mask[:, i] = torch.tensor(binary_num, dtype=torch.bool, device=seq.device)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    pos_seq=torch.arange(K)/K
    pos_seq=pos_seq.unsqueeze(dim=-1) # (K,1)
    y=pos_seq.repeat(1,M).unsqueeze(dim=0) # (1,K,M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    temp = torch.arange(M)
    temp = torch.div(temp, M,rounding_mode="floor").reshape(1, -1)
    position = torch.arange(K).unsqueeze(dim=-1).float()
    y = torch.zeros(1, K, M)

    # 计算偶数索引和奇数索引对应的除数
    even_divisor = torch.pow(10000, temp[0, 0::2])
    odd_divisor = torch.pow(10000, temp[0, 1::2])
    # 注意：切片语法中的双冒号 (即 [::2]) 表示从索引 0 开始，选择每隔一个的元素。
    y[0, :, 0::2] = torch.sin(position / even_divisor)
    y[0, :, 1::2] = torch.cos(position / odd_divisor)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
