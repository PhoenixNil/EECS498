...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        out = torch.mm((x.view(x.shape[0], -1)), w)+b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
...
class ReLU(object):
    ...
    def backward(dout, cache):
        ...
        #####################################################
        # Replace "pass" statement with your code
        dx = torch.where(x > 0, dout, torch.zeros_like(x))
        #####################################################
        #                  END OF YOUR CODE                 #
        #####################################################
        return dx
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        hidden, relucahe = Linear_ReLU.forward(
            X, self.params['W1'].double(), self.params['b1'].double())
        scores, fccache = Linear.forward(
            hidden, self.params['W2'].double(), self.params['b2'].double())
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        loss = dataloss+self.reg * \
            (torch.sum(self.params['W1']**2)+torch.sum(self.params['W2']**2))
        dh, dw2, db2 = Linear.backward(dx, fccache)
        grads['W2'] = dw2+self.reg*self.params['W2']
        grads['b2'] = db2
        dx, dw1, db1 = Linear_ReLU.backward(dh, relucahe)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            if i == 0:
                self.params['W'+str(i)] = torch.randn(
                    input_dim, hidden_dims[i], dtype=dtype, device=device)*weight_scale
                self.params['b'+str(i)] = torch.zeros(
                    hidden_dims[i], dtype=dtype, device=device)
            elif i == self.num_layers-1:
                self.params['W'+str(i)] = torch.randn(
                    hidden_dims[i-1], num_classes, dtype=dtype, device=device)*weight_scale
                self.params['b'+str(i)] = torch.zeros(
                    num_classes, dtype=dtype, device=device)
            else:
                self.params['W'+str(i)] = torch.randn(
                    hidden_dims[i-1], hidden_dims[i], dtype=dtype, device=device)*weight_scale
                self.params['b'+str(i)] = torch.zeros(
                    hidden_dims[i], dtype=dtype, device=device)
        # #######################################################################
        #                         END OF YOUR CODE                            #
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            if i == self.num_layers-1:
                scores, cache['fc'+str(i)] = Linear.forward(
                    out, self.params['W'+str(i)], self.params['b'+str(i)])
            else:
                out, cache['fc'+str(i)] = Linear.forward(
                    out, self.params['W'+str(i)], self.params['b'+str(i)])
                out, cache['relu'+str(i)] = ReLU.forward(out)
                if self.use_dropout:
                    out, cache['dropout'+str(i)] = Dropout.forward(
        ...
        for i in range(self.num_layers): # add regularization
            loss += self.reg*torch.sum(self.params['W'+str(i)]**2)



        for t in range(self.num_layers , 0, -1):
            i=t-1
            if t == self.num_layers: # last layer
                dout, grads['W'+str(i)], grads['b'+str(i)] = Linear.backward(
                    dscores, cache['fc'+str(i)])
                grads['W'+str(i)] += self.reg*2*self.params['W'+str(i)]
            else:
                if self.use_dropout:
                    dout = Dropout.backward(dout, cache['dropout'+str(i)])
                dout = ReLU.backward(dout, cache['relu'+str(i)])
                dout, grads['W'+str(i)], grads['b'+str(i)] = Linear.backward(
                    dout, cache['fc'+str(i)])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver=Solver(model,data_dict,device=device, lr_decay=0.90, 
...
def sgd_momentum(w, dw, config=None):
    ...
    ##################################################################
    # Replace "pass" statement with your code
    v = config['momentum']*v - config['learning_rate']*dw  #累积梯度,更新动量项v。通过将当前梯度dw乘以一个动量因子（通常介于0和1之间的值），然后将其加到之前的动量项v上，得到新的动量项v_new。
    next_w = w + v
    ###################################################################
    #                           END OF YOUR CODE                      #
    ###################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand_like(x) > p).to(x.device)
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
