...
def train_linear_classifier(loss_func, W, X, y, learning_rate=1e-3,
    ...
    if W is None:
        ...
        W = 0.000001 * torch.randn(dim, num_classes,
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        for j in range(num_class):
            dW[:, j] += torch.exp(scores[j]) / sum_exp_scores * X[i]
        dW[:, y[i]] -= 1.0 * X[i]
        loss += torch.log(sum_exp_scores) - scores[y[i]]
    dW /= num_train
    loss /= num_train
    loss += reg * torch.sum(W * W)
    dW += reg * 2 * W
...
def softmax_loss_vectorized(
    ...
    num_class = W.shape[1]
    score = torch.mm(X, W)
    score -= torch.max(score, dim=1, keepdim=True)[0] # numeric stability
    score_exp = torch.exp(score)
    sum_exp_scores = score_exp.sum(dim=1)
    loss = torch.log(sum_exp_scores).sum() - \
        score[torch.arange(X.shape[0]), y].sum()
    loss /= num_train
    loss += reg*torch.sum(W*W)
