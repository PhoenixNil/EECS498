...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        # Create stem layers for class prediction
        #需要分别创建，不然他们引用的都是同一个object，对1个修改会影响另外一个
        for out_channels in stem_channels:
            conv = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1)
            nn.init.normal_(conv.weight, mean=0.0, std=0.01)
            nn.init.zeros_(conv.bias)
            relu = nn.ReLU()
            stem_cls.extend([conv, relu])
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key,level_feature in feats_per_fpn_level.items():
            ...
            temp1 = self.pred_cls(output) #output (batch_size, fpn_channels, H, W)
            class_logits[f'{key}'] = temp1.view(temp1.shape[0],temp1.shape[1],-1).permute(0,2,1) #output (batch_size, H * W, num_classes)

            for layer in self.stem_box:
                output = layer(level_feature)
            temp2 = self.pred_box(output) #output (batch_size, fpn_channels, H, W)
            boxreg_deltas[f'{key}'] = temp2.view(temp2.shape[0],temp2.shape[1],-1).permute(0,2,1) #output (batch_size, H * W, 4)
            temp3 = self.pred_ctr(output) #output (batch_size, fpn_channels, H, W)
            centerness_logits[f'{key}'] = temp3.view(temp3.shape[0],temp3.shape[1],-1).permute(0,2,1) #output (batch_size, H * W, 1)
...
def fcos_get_deltas_from_locations(
    ...
    l = (locations[:, 0] - gt_boxes[:, 0]) / stride #l = (xc - x1) / stride
    t = (locations[:, 1] - gt_boxes[:, 1]) / stride #t = (yc - y1) / stride
    r = (gt_boxes[:, 2] - locations[:, 0]) / stride #r = (x2 - xc) / stride
    b = (gt_boxes[:, 3] - locations[:, 1]) / stride #b = (y2 - yc) / stride
...
def fcos_apply_deltas_to_locations(
    ...
    deltas = deltas.clamp(min=0) # clamp deltas to zero
    # deltas[deltas.sum(dim=1) == -4, :] = 0
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = None
    # Replace "pass" statement with your code
    min_lr=torch.min(deltas[:,0],deltas[:,2])
    min_tb=torch.min(deltas[:,1],deltas[:,3])
    max_lr=torch.max(deltas[:,0],deltas[:,2])
    max_tb=torch.max(deltas[:,1],deltas[:,3])
    centerness=torch.sqrt((min_lr*min_tb)/(max_lr*max_tb))
    centerness[deltas.sum(dim=1) == -4] = -1
    centerness = centerness.to(device=deltas.device, dtype=deltas.dtype)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        feats_per_fpn_level=self.backbone(images) #fpn和backbone 
  
        pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits = self.pred_net(feats_per_fpn_level) #head
        ...
        locations_per_fpn_level = get_fpn_location_coords(fpn_feats_shapes, 
                                    self.backbone.fpn_strides, device=images.device) #计算p特征图中心点坐标
        ...
        matched_gt_boxes = []
        # Replace "pass" statement with your code
        for batch in range(gt_boxes.shape[0]): #B张图片，n个目标,每个目标是（x1,y1,x2,y2,c)
          matched_boxes_per_fpn_level=fcos_match_locations_to_gt(locations_per_fpn_level,self.backbone.fpn_strides,gt_boxes[batch])
        ...
        for batch in range(gt_boxes.shape[0]):
          temp_dict={}
          for i in ['p3','p4','p5']:
            matched_boxes_per_fpn_level_deltas=fcos_get_deltas_from_locations(locations_per_fpn_level[i],
            matched_gt_boxes[batch][i],self.backbone.fpn_strides[i]) #这个只计算每张图片第i层的deltas
            temp_dict[i]=matched_boxes_per_fpn_level_deltas  #将所有层的deltas放到一个字典里
          matched_gt_deltas.append(temp_dict)
        ...
        # 将类别索引从0-based转换为1-based
        matched_gt_classes = matched_gt_boxes[:, :, 4] + 1

        # 将类别索引转换为one-hot编码
        matched_gt_classes_one_hot = torch.nn.functional.one_hot(
            matched_gt_classes.to(torch.int64),
            num_classes=21
        )

        # 去除背景类别列
        classes = matched_gt_classes_one_hot[:, :, 1:]

        # 计算分类损失
        loss_cls = sigmoid_focal_loss(pred_cls_logits, targets=classes.float())
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            # Replace "pass" statement with your code
            classes = level_pred_scores.argmax(dim=1) ##取得N个边界框最高分数所在的索引，N是边界框
            level_pred_scores = level_pred_scores.max(dim=1).values #每个边界框的最高分数

            # Step 2:
            # Replace "pass" statement with your code
            keep=level_pred_scores>test_score_thresh #only retain higher threshold box
            ...
            # Step 3:
            # Replace "pass" statement with your code
            level_pred_boxes=fcos_apply_deltas_to_locations(level_deltas[keep],
            level_locations[keep],self.backbone.fpn_strides[level_name]) #return coord for match box
