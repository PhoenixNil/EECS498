...
def nn_forward_backward(
    ...
    data_loss=torch.log(sum_exp_scores).sum()-scores[torch.arange(N), y].sum()
    data_loss/=N
    reg_loss = reg * ((W1 * W1).sum() + (W2 * W2).sum())
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    scores, hidden = nn_forward_pass(params, X)  # N x C
    y_pred = torch.argmax(scores, dim=1)  # N x 1
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1.1,1,0.9995]
    hidden_sizes = [256]
    regularization_strengths = [0.001,5e-4,1e-5]
...
def find_best_net(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    learning_rates, hidden_sizes, regs, learning_rate_decays = get_param_set_fn()
    for lr in learning_rates:
        for hs in hidden_sizes:
            for r in regs:
                for lrd in learning_rate_decays:
                    net = TwoLayerNet(
                        3 * 32 * 32, hs, 10, device=data_dict['X_train'].device, dtype=data_dict['X_train'].dtype)
                    stat = net.train(
                        data_dict["X_train"],
                        data_dict["y_train"],
                        data_dict["X_val"],
